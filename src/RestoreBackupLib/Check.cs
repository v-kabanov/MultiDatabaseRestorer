...
namespace RestoreBackupLib
{
	...
	{
		private static readonly ILog Log = LogManager.GetLogger(typeof(Check).Name);
		...
		public static void DoAssertLambda(bool assertion, Func<string> message)
		{
			if (UseExceptions)
			{
				if (!assertion) throw new PreconditionException(message.Invoke());
			}
			else
			{
		...
		/// </summary>
		private static bool UseExceptions => !_useAssertions;
